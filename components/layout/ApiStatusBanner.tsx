  ...
  return (
    <AnimatePresence>
      {error && (
        ...
        >
          <div className="glass-strong flex max-w-xl items-start gap-3 rounded-2xl border border-brand-400/30 px-4 py-3 text-sm text-brand-100">
            <span aria-hidden>⚠️</span>
            <div className="min-w-0">
              <p>{error}</p>
              <p className="mt-0.5 text-xs text-white/40">API: {API_BASE_URL}</p>
            </div>
            <button
              onClick={clearError}
